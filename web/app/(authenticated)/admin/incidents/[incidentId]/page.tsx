...
export default async function AdminIncidentDetailPage({
    ...
    return (
      <main className="homeLayout">
        <PageHeader
          ...
          summary="Incident detail retrieval failed for this incident identifier."
  ...
  const timelinePreview = timelineResult.ok && timelineResult.data
    ? timelineResult.data.items.slice(0, 5)
    : [];
    ...
    <main className="homeLayout">
      <PageHeader
        ...
        secondaryActions={[
          { href: adminIncidentsPath, label: "Back to incidents" },
          { href: adminIncidentStatusPath, label: "Incident status" },
          { href: adminIncidentTimelinePath(incident.id), label: "Open timeline" }
        ]}
        summary="Incident command summary with timeline handoff for launch and rollback readiness."
        title={incident.id}
      />
      ...
      <section className="sectionCard ukde-panel">
        <div className="auditIntegrityRow">
          ...
          <StatusChip tone={statusTone(incident.status)}>{incident.status}</StatusChip>
        </div>
      ...
      <section className="sectionCard ukde-panel">
        ...
        {!timelineResult.ok ? (
          <SectionState
            ...
            description={timelineResult.detail ?? "Unable to load incident timeline preview."}
          />
        ...
        <div className="buttonRow">
          <Link className="secondaryButton" href={adminIncidentTimelinePath(incident.id)}>
            Open full timeline
          </Link>
        </div>
      </section>
