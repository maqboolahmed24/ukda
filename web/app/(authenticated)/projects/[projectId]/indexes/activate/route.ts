...
import { activateProjectIndex } from "../../../../../../lib/indexes";
import {
...
export async function POST(
  ...
  const result = await activateProjectIndex(projectId, kind, indexId);
  if (!result.ok || !result.data) {
    return redirectTo(`${projectIndexesPath(projectId)}?status=action-failed`);
  }
  revalidateAfterMutation("indexes.activate", {
    projectId,
    indexKind: kind,
    indexId
  });
  return redirectTo(`${detailPath(projectId, kind, indexId)}?status=activated`);
}
